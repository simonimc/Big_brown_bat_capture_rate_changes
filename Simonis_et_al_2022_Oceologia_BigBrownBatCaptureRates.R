...
EPFUcount<- EPFU_data %>% 
  select(state, site_mask, month, year, age, sex, repstat, years_Pd, disease_time_step, pre_post, disease_status, lat, lon, rMapCounty)  

EPFUcount<- count(EPFUcount, vars = c('state', 'site_mask', 'month', 'year', 'sex', 'age', 'repstat', 'years_Pd', 'disease_time_step', 'count_cent_lat', 'count_cent_lon', 'rMapCounty')) %>% 
  group_by(year, site)
...
get_prior(freq~sex*disease_time_step*county_cent_lat + (1|site_mask), data = adult_EPFUcount, family = poisson())
...
adult_count_mod1<- brm(freq ~ sex*disease_time_step*county_cent_lat + (1|site_mask), data = adult_EPFUcount, family = poisson(), prior = adult_prior, chains = 4, warmup = 5000, iter = 10000, sample_prior = 'yes', control = list(max_treedepth = 15))
...
cond_ef_adult_count1<- emmeans(adult_count_mod1, ~sex*disease_time_step*county_cent_lat)
...
get_prior(freq~county_cent_lat*repstat*disease_time_step + (1|site_mask), data = fem_EPFUcount, family = poisson())

fem_prior<-set_prior("student_t(3, 0.7, 2.5)", class = "Intercept")


#Create initial model for females 
fem_count_mod1<- brm(freq ~ county_cent_lat*repstat*disease_time_step + (1|site_mask), data = fem_EPFUcount, family = poisson(), prior = fem_prior, chains = 4, warmup = 5000, iter = 10000, sample_prior = 'yes', control = list(max_treedepth = 15))
...
cond_ef_fem_count1<- emmeans(fem_count_mod1, ~county_cent_lat*disease_time_step*repstat)
...
adult_EPFUcount$lat_cat<- ifelse(adult_EPFUcount$lat > 39.3, 'north', 'south')
...
fem_EPFUcount$lat_cat<- ifelse(fem_EPFUcount$county_cent_lat > 39.3, 'north', 'south')
